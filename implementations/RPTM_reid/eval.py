...
from eval_reid import gen_clustered_data, eval_cluster


def do_test(
    model, queryloader, galleryloader, batch_size, use_gpu, dataset, ranks=[1, 5, 10]
):
    ...
    print(
        "=> BatchTime(s)/BatchSize(img): {:.3f}/{}".format(batch_time.avg, batch_size)
    )
    ...
    print("Computing CMC and mAP")
    if dataset == "vehicleid":
        cmc_re, mAP_re = evaluate_vid(
            distmat_re, q_pids, g_pids, q_camids, g_camids, 50
        )
    else:
    ...
    pred_clusters = eval_cluster(gf, g_pids)

    gen_clustered_data(
        5,
        pred_clusters,
        200,
        "./data/veri/image_test/",
        "./logs/veri/clusters",
        "./data/veri/name_test.txt",
    )
