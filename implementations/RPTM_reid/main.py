...
def main():
    parser = argparse.ArgumentParser(
        description="Relation Preserving Triplet Mining for Object Re-identification"
    )
    parser.add_argument(
        "--config_file",
        default="configs/veri_r101.yml",
    ...
    dataset_kwargs, transform_kwargs, optimizer_kwargs, lr_scheduler_kwargs = (
        return_kwargs(cfg)
    )
    ...
    trainloader, train_dict, data_tfr, testloader_dict, dm = data_loader(
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=cfg.SOLVER.STEPSIZE, gamma=cfg.SOLVER.GAMMA
    )

    criterion_xent = CrossEntropyLoss(
        num_classes=train_dict["num_train_pids"], use_gpu=use_gpu, label_smooth=True
    )
    ...
    if cfg.TEST.EVAL:
        ...
        for name in cfg.DATASET.TARGET_NAME:
            ...
            _, distmat, _, distmat_re = do_test(
                model,
                queryloader,
                galleryloader,
                cfg.TEST.TEST_BATCH_SIZE,
                use_gpu,
                cfg.DATASET.TARGET_NAME[0],
            )
