...
parser.add_argument(
    "--querylist",
    default="./list/veri_query_list.txt",
    type=str,
    metavar="DIR",
    help="path to query list",
)
parser.add_argument(
    "--gallerylist",
    default="./list/veri_test_list.txt",
    type=str,
    metavar="DIR",
    help="path to gallery list",
)
parser.add_argument(
    "--queryFeat",
    default="./results/veri/resnet101/queryFeat.npy",
    type=str,
    metavar="DIR",
    help="path to query feature",
)
parser.add_argument(
    "--galleryFeat",
    default="./results/veri/resnet101/galleryFeat.npy",
    type=str,
    metavar="DIR",
    help="path to gallery feature",
)
...
def main():
    ...
    distmat = np.sqrt(
        getNormMatrix(query_feat_mat, gallery_feat_mat.shape[0]).T
        + getNormMatrix(gallery_feat_mat, query_feat_mat.shape[0])
        - 2 * np.dot(query_feat_mat, gallery_feat_mat.T)
    )
    print("Dist mat: ", distmat.shape)
    # np.savetxt('distmat.txt', distmat, fmt='%.4f')

    cmc, mAP = eval_func(
        distmat, query_pids, query_camids, gallery_pids, gallery_camids, max_rank=100
    )
